...
* `41589_2019_400_MOESM31_ESM.xlsx` - original Excel file from the BiG-SCAPE paper detailing their manually curated dataset
* `pfam_annotation_total_proG3/labels.tsv` - labels associated with the Pfam annotation of the proG3 dataset
* `biosynthetic_classes.tsv` - the biosynthetic class annotation of the BGCs from the proG3 dataset
* `pfam_annotation_prophage/labels.tsv` - labels associated with the Pfam annotation of the prophage dataset
...
benchmark=read.delim(paste0("average_0.80.tsv"))
...
htgcf_f=htgcf[-temp,]
merged_data=merge(sep_curated,htgcf_f,by="mibig", all.x = TRUE)
...
pro_igua=read.delim('supplementary_table_17.tsv', header=TRUE)
...
sec_igua=read.delim('supplementary_table_13.tsv', header=TRUE)
...
proG3_igua=read.delim('supplementary_table_11.tsv', header=TRUE)
...
ggarrange(
  ggarrange(bgcs_hist_GCs, sec_hist_GCs, pro_hist_GCs, align="hv",
            ncol = 3, labels = c("A", "E", "I")),
  ggarrange(bgcs_area_size, sec_area_size, pro_area_size, align="hv",
            ncol = 3, labels = c("B", "F", "J")),
  ggarrange(bgcs_umap,sec_umap,pro_umap,
            align = "hv", ncol = 3,
            labels = c("C", "G", "K")),
  ggarrange(bgcs_hist_phyla, sec_hist_phyla, pro_hist_phyla,
            align = "hv", ncol = 3,
            labels = c("D", "H","L")),
  align = "hv", heights = c(0.5,0.65,2,0.5), nrow = 4
)
```
...
big_scape_glocal <- read.delim("supplementary_table_2.tsv", header=FALSE, comment.char="#", stringsAsFactors=TRUE)
...
big_scape <- read.delim("supplementary_table_5.tsv", header=FALSE, comment.char="#")
...
igua_strepto <- read.delim("supplementary_table_6.tsv")
...
labels <- read.delim("pfam_annotation_total_proG3/labels.tsv", header=FALSE)
...
##Panel B - waiting for re-writing
...
prophage_metadata <- read.delim("prophage_metadata.tsv", header=TRUE)
labels <- read.delim("pfam_annotation_prophage/labels.tsv", header=FALSE)
