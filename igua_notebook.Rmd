...
##For all panels - setting up separate data frame for plotting

#Importing the secretion systems Seurat object

pfams_sec <- readRDS("pfams_secretion.RDS")

#Setting the colour palettes
...
reduced_cols=c("#15889C", "#FFB300", "#FF6F00" ,"#43A047")
...
#Re-running clustering with higher (default) resolution

pfams_sec_default_res <- FindClusters(object = pfams_sec, verbose = FALSE, algorithm = 3)

#Extracting relevant information from the Seurat object to a data frame
#This is to allow for plotting with ggplot2

df=as.data.frame(pfams_sec_default_res@reductions[["umap"]]@cell.embeddings)
df$secretion=rownames(df)


df2=pfams_sec_default_res@meta.data

df2=df2 %>% 
  select(seurat_clusters, major_secretion_system_type, minor_secretion_system_type,
         Majority, secretion)

df3=merge(df, df2, by="secretion")

#Make panels A, B and C

p1=ggplot(df3, aes(x=umap_1, y=umap_2, color=Majority))+
  geom_point(alpha=0.5, size=2)+
  theme_classic()+
  scale_color_manual(values=precols)+
  theme(legend.position = "bottom")+
  theme(legend.title = element_blank())

p2=df3 %>%
  filter(between(umap_1, 2, 4), between(umap_2, -14, -8.5)) %>%
  ggplot(aes(umap_1, umap_2, colour = Majority)) +
  geom_point(alpha=0.5, size=2)+
  theme_classic()+
  scale_color_manual(values = reduced_cols)+
  theme(legend.title = element_blank())


p3=df3 %>%
  filter(between(umap_1, 2, 4), between(umap_2, -14, -8.5)) %>%
  ggplot(aes(umap_1, umap_2, colour = seurat_clusters)) +
  geom_point(alpha=0.5, size=2)+
  theme_classic()+
  scale_color_manual(values = c("#33BBEE", "#EE3377", "#009988"))+
  theme(legend.title = element_blank())

#Make panel D

cluster_10=df3 %>% 
  filter(grepl('10', seurat_clusters)) %>% 
  group_by(major_secretion_system_type) %>% 
  summarise(n=n())

cluster_22=df3 %>% 
  filter(grepl('22', seurat_clusters)) %>% 
  group_by(major_secretion_system_type) %>% 
  summarise(n=n())

df4=rbind(cluster_10, cluster_22)

df4$Cluster=c(rep("10", 3), rep("22",3))

df4$Cluster=as.factor(df4$Cluster)

p4=ggplot(df4, aes(y=n, x=Cluster, fill=major_secretion_system_type))+
  geom_bar(position = "fill", stat = "identity")+
  scale_fill_manual(values = reduced_cols)+
  theme_classic()+
  theme(legend.title = element_blank())+
  xlab("")+
  ylab("Proportion of GCFs")

#Make and export the final panel figure

figure_4=ggarrange(p1,
          ggarrange(p2, p3, p4,ncol = 3, nrow = 2,
                    align="hv", widths = c(2,2,1.5), labels = c("", "C", "D")),
          ncol=2, align="hv", widths = c(2,3), labels = c("A", "B")
)


ggsave("figure_4.svg", plot = new_sec_figure, dpi = 350, height = 7, width = 14)

#Please note that boxes, lines and the clinker figure were added manually in Inkscape.
```
